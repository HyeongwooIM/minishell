...
void    make_token_lst(t_token *chunks, t_token *tokens)
{
    ...
    while(chunks)
    {
        if (chunks->type == CHAR)
        {
            if (!have_cmd_type(tokens))
                add_node(CMD, chunks->word, tokens);
            else
                add_node(OPTION, chunks->word, tokens);
        }
        else if (chunks->type == REDIRECT || chunks->type == PIPE)
